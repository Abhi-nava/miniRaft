...
# PLAYGROUND ENDPOINTS — crash/recover simulation

@app.post("/crash")
async def crash_replica():
    """
    Simulate a replica crash by stopping its Docker container.
    ...
    log.warning(f"🔴 REPLICA {REPLICA_ID} CRASHING - stopping containers")
    
    try:
        # Stop replica container
        subprocess.run(["docker", "stop", replica_container], check=False, timeout=5)
        # Stop MongoDB container
        subprocess.run(["docker", "stop", mongo_container], check=False, timeout=5)
...
async def recover_replica():
    """
    Recover from a crash by restarting the Docker containers.
    Used for RAFT playground testing.
    """
    replica_container = f"jackfruit-replica{REPLICA_ID}-1"
    mongo_container = f"jackfruit-mongo{REPLICA_ID}-1"
    
    log.warning(f"🟢 REPLICA {REPLICA_ID} RECOVERING - restarting containers")
    
    try:
        # Start MongoDB container first
        subprocess.run(["docker", "start", mongo_container], check=False, timeout=5)
        # Give MongoDB a moment to start
        await asyncio.sleep(2)
        # Start replica container
        subprocess.run(["docker", "start", replica_container], check=False, timeout=5)
